# Style Components
* makes CSS Scoped to components, so you do not have to worry if you have used a class somewhere already
* required in package.json dependencies: "styled-components": "^5.3.6"

Apply global style: rename style.css -> style.js
```jsx
import { createGlobalStyle } from "styled-components"  // Necessary to build css into JS; but name is 
import {Open_Sans} from "next/font/google"             // Google Fonts is also built into Next.js

cons mainFont = Open_Sans({subset:["latin"]})          // for the font to work you have to define some Parameters
...
export default createGlobalStyle`   
  ...
  // ... CSS here wrapped by Template String
  :root{
...
* Whatever I apply to the Container, it applies to all Elements inside the Container
```jsx
// in the App
import Container from "../Components/Container"

<Component />
<Component direction="column" />  // Passing the prop of column to "direction"
...
export default styled.div`
  ...
  gap:2rem;
  background:blue;
  flex-direction: ${({direction})=> direction === "column" ? "column" : "row"};   // Checking what prop was passed into "direction", therefore destructure direction with {}
  // Short Version: ${({ direction = "row" }) => direction };
  
    &:hover{                     // Hover as nested Style, can also accept variables as the code above 
...
**Styling a Next-Component**
* note: when using the Link-component e.g. in the App, you have to import the one you styled, not the one from Next, otherwise styles will not show up
* you could also rename the exports, e.g. "A" or "Kartoffelsalat" :)
```jsx
import styled from "styled-components"
import Link from "next/link"      // import the Component
  
export default styled(Link)`       // Will also work for Image, etc.
...
const StyledSection = styled.section`
  ...
  ${({variant}) => {            // using conditions through props in the styled section
    variant === "success"   
    ? css`
        color:var(--primary-color);       // these styles will add & potentially override above styles
        border-olor: var(--primary-color);
        `
      : null
  }}
`
...
// in the app.js
<InfoBox title="Here is the Title text" message="This is my copy text that will be in the element" />
```
