...
import { movies } from "../../Data/movies"; // adding the movies
import { MovieCard } from "../MovieCard/MovieCard";
//console.log("movies", movies); // logs an array with 14 movies, great everything works
...
export default function Movies() {
  return (
    ...
    //   <section className="movies__movies">
          ...
          <h1 key={movie.id}>{movie.title}</h1>) // every item that was created from an array needs a key
          // dont forget the return statement!!
        })} */

          // Destructure properties already when we want to use them, we dont know how big the Object is
          {movies.map((movie) => {
            return (
              <MovieCard
                key={movie.id}
