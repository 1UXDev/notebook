...
## Working with Immer
-> add "use-immer and "immer" to package.json
...
updateSeasons((draft) => {
  // use draft keyword with immer
  const season = draft.find(({ number }) => number === seasonNumber); // destructure object to directly access the attribute we need

  const episode = draft.find(({ number }) => number === episodeNumber);

  episode.hasSeen != episode.hasSeen; // with immer we do not have to 2xMap but can do this, which we normally could not in react
});
```
